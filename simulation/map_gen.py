...
class beh_beacons(object):
	...
	def __init__(self, size):
		...
		self.positions = np.random.randint(-40, 40, (size, 2))
...
class swarm(object):
	...
	def iterate(self, noise):
		...
		flocking(self, 0.2, 0.2, 10, 0.3, noise)
		# # flocking_infoGain(self, 0.1, 0.05, 5, 0.9, noise)

		force = 30

		# force should be 20 for social exploration
		# dispersion(self, np.array([0,0]), force, noise)
...
def robot_malicious(swarm, noise):
...
def social_behaviour(swarm, vector, param, noise):

	R = param; r = 2; A = 1; a = 20

	states = swarm.behaviour == 90
...
def dispersion(swarm, vector, param, noise):
	...
	a += Avoid - vector + noise
	
	vecx = a.T[0]
	vecy = a.T[1]
	angles = np.arctan2(vecy, vecx)

	# Add error to heading angles
	...
	Wx = swarm.speed*swarm.motor_speeds*np.cos(angles)
	Wy = swarm.speed*swarm.motor_speeds*np.sin(angles)
	# print('Swarm motor speeds: ', swarm.speed*swarm.motor_slow)
